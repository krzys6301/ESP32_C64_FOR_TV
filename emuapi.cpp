...
int emu_ReadKeys(void)
{
  ...
  if ((gpio_get_level((gpio_num_t)JOY_swap) == 1) != joySwapped) {
    joySwapped = !joySwapped;
    Serial.printf("joySwapped %d\n", joySwapped);
  }
  if (joySwapped) {
    retval = ((j1 << 8) | j2);
  }
...
void emu_InitJoysticks(void) {
  ...
  gpio_set_direction((gpio_num_t)JOY_swap, GPIO_MODE_INPUT);
  gpio_set_pull_mode((gpio_num_t)JOY_swap, GPIO_PULLUP_ONLY);
